...
def lambda_handler(event, context):
    ...
    # Prepare CSV content
    csv_lines = []
    csv_lines.append("Service Name,Amount (USD)")
    ...
    for group in response["ResultsByTime"][0]["Groups"]:
        ...
        total += amount
        line = '"{}",${:,.2f}'.format(service_name, amount)  # wrap service_name in quotes in case it contains commas
        print(line)
        csv_lines.append(line)

    #csv_lines.append('Total,${:,.2f}'.format(total))
    csv_lines.append('Total,"${:,.2f}"'.format(total))
...
def send_email(month, attachment):
    ...
    part.add_header('Content-Disposition', 'attachment',
                    filename="AWS-MonthlyCostByService-{}.csv".format(month))
    msg.attach(part)
